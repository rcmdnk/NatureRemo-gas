...
function fillNatureRemo(){
  ...
  var devices_data = devices();
  var appliances_data = appliances();
  columns = columns.concat(devices_data['columns']);
  columns = columns.concat(appliances_data['columns']);
  data = data.concat(devices_data['data']);
...
function devices() {
  var data = getNatureRemoData("devices");
  var events = data[0].newest_events;
  return {'columns': ['Temperature'], 'data': [events.te.val]};
}